...
err_flag init_dynarr_nodes( er_dynarr_nodes * arrn, uint32_t size){
  def_err_handler(!arrn, "init_dynarr", ERR_NULL);

  arrn->elems = calloc(size , sizeof(struct s_graph_entry *));
  def_err_handler(!arrn->elems, "init_dynarr", ERR_ALLOC);
